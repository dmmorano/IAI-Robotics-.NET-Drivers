...
#End Region
    ...
    Public State As SCONState
    Public PollingActive = True
    Public PollingRate = 50

    Private singleCoilWriteQueue As Concurrent.ConcurrentQueue(Of KeyValuePair(Of Integer, Boolean))
    Private singleRegisterWriteQueue As Concurrent.ConcurrentQueue(Of KeyValuePair(Of Integer, Integer))

    Private singleCoilReadQueue As Concurrent.ConcurrentQueue(Of Integer)
    Private singleRegisterReadQueue As Concurrent.ConcurrentQueue(Of Integer)

    Private multipleCoilReadQueue As Concurrent.ConcurrentQueue(Of Integer)
    Private multipleRegisterReadQueue As Concurrent.ConcurrentQueue(Of Integer)

    Private serialQueue As Concurrent.ConcurrentQueue(Of Threading.Thread)

    Private getStateThread As Threading.Thread
...
#Region "State monitoring thread"
    Private Sub startGetStateThread()
        getStateThread = New Threading.Thread(AddressOf getStateLoop)
        getStateThread.Start()
    End Sub

    Private Sub getStateLoop()
        While True
            If PollingActive Then
                GetState()
            End If
            Threading.Thread.Sleep(PollingRate)
        End While
    End Sub
...
#Region "Public Methods"
    ...
    ''' <summary>
    ''' Connect modbus client with previously declared port and baudrate. Routine will throw exception if either are not declared.
    ''' </summary>
    Public Sub Connect()
        If modbusClient.Connected Then
            modbusClient.Disconnect()
...
#Region "Private Methods"
    ...
    Private Function parseState(statusCoilData, statusRegData) As SCONState
        Dim state As New SCONState With {
            ...
            .PNOW = (statusRegData(0) * &H10000) + statusRegData(1) * 0.01,
            ...
            .STAT = (statusRegData(8) * &H10000) + statusRegData(9),
            .VNOW = (statusRegData(10) * &H10000) + statusRegData(11) * 0.01,
            .CNOW = (statusRegData(12) * &H10000) + statusRegData(13),
            .STIM = (statusRegData(14) * &H10000) + statusRegData(15)
        }
    ...
    Private Function writeSingleCoil(coil As Integer, value As Integer) As Boolean
        ...
        If modbusClient.Connected Then
            Try
                ...
                success = True
            Catch ex As Exception
                Console.WriteLine("Failure to write holding register: " & coil & " " & ex.Message)
                success = False
            End Try
    ...
    Private Function writeMultipleHoldingRegisters(start As Integer, values As Integer()) As Boolean
        ...
        If modbusClient.Connected Then
            Try
                modbusClient.WriteMultipleRegisters(start + 1, values)
                success = True
            Catch ex As Exception
                Console.WriteLine("Failure to write multiple holding registers: " & start & " " & ex.Message)
                success = False
            End Try
    ...
    Private Function writeMultipleCoils(start As Integer, values As Boolean()) As Boolean
        ...
        If modbusClient.Connected Then
            Try
                modbusClient.WriteMultipleCoils(start + 1, values)
                success = True
            Catch ex As Exception
                Console.WriteLine("Failure to write holding register: " & start & " " & ex.Message)
                success = False
            End Try
...
Public Class SCONState
    Public EMGS As Boolean      'EMG status 
    Public SFTY As Boolean      'Safety speed enabled status
    Public PWR As Boolean       'Controller ready status 
    Public SV As Boolean        'Servo On status 
    Public PSFL As Boolean      'Missed work In push-motion operation 
    Public ALMH As Boolean      'Major failure status 
    Public ALML As Boolean      'Minor failure status 
    Public ABER As Boolean      'Absolute Error status ABER
    Public BKRL As Boolean      'Brake forced-release status 
    Public STP As Boolean       'Pause status 
    Public HEND As Boolean      'Home Return status 
    Public PEND As Boolean      'Position complete status 
    Public LOAD As Boolean      'Load output judgment status 
    Public TRQS As Boolean      'Torque level status 
    Public MODS As Boolean      'Teaching mode status 
    Public TEAC As Boolean      'Position-data load command status 
    Public JOGUp As Boolean     'Jog+ status 
    Public JOGDn As Boolean     'Jog- status  

    Public PNOW                  'Current Position Monitor (high Byte)
    'Public PNOW                 'Current Position Monitor (low byte)
    Public ALMC                  'Present alarm code query
    Public DIPM                  'Input port query
    Public DOPM                  'Ouput port monitor query
    Public DSS1                  'Device status query 1
    Public DSS2                  'Device status query 2
    Public DSSE                  'Expansion device status query
    Public STAT                  'System status query (high Byte)
    'Public STAT                 'System status query (low byte)
    Public VNOW                  'Current speed monitor (high Byte)
    'Public VNOW                 'Current speed monitor (low byte)
    Public CNOW                  'Current ampere monitor (high Byte)
    'Public CNOW                 'Current ampere monitor (low byte)
    Public DEVI                  'Deviation Monitor (high Byte)
    'Public DEVI                 'Deviation Monitor (low byte)
    Public STIM                  'System timer query (high Byte)
